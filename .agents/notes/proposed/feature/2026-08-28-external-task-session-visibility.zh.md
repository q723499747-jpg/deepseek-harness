...
拥有持久 event type 的仓外插件通过 `SessionStore.registerEventTypes()` 为自身 plugin effect 注册完整名称。只有至少一个 owner 注册有效时，persistence 才接受这些事件；其他外部事件仍失败关闭。log-only owner 可以在同一边界声明 exact 历史 `ignorable:true` 信封。restore 只从 detached 内存副本移除该标记，绝不修补归档，其他任意键或值仍不受支持。这样，精确 hydrate 可以恢复已安装插件的日志，又不会削弱未知事件 guard。
