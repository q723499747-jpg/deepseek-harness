...
An out-of-repository plugin that owns durable event types registers their complete names through `SessionStore.registerEventTypes()` for its plugin effect. Persistence accepts those events only while at least one owner registration is active; every other external event still fails closed. A log-only owner can declare the exact historical `ignorable:true` envelope at the same boundary. Restore removes only that marker from its detached in-memory copy; it never repairs the archive, and every other key or value remains unsupported. This lets exact hydration restore installed plugin logs without weakening the unknown-event guard.
