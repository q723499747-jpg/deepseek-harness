...
- **`SESSION_FORMAT_VERSION` stays pinned at `0`** — pre-release, no broad compatibility implied: `Session` accepts only current seed shapes, a backend refuses any other version, and every event type not built in or registered by a currently loaded owning plugin refuses reconstruction. An owner may declare only the exact historical `ignorable:true` envelope for its log-only vocabulary; other extension values or keys remain unsupported, and restoration never rewrites the archive ([mechanism](../../../.agents/notes/implemented/simplification/2026-08-25-fail-closed-session-event-vocabulary.md)).
