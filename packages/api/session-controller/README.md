...
Host integrations that execute an external task without a model turn call `ctx.sessionController.markExternalTaskVisible(session, marker)`. It appends one idempotent `session/external-task` log-only marker, so Session listing and restart recovery retain the task while model history remains unchanged; callers continue to own their task events and visible cards.

An integration that already owns a durable Session can separately call `ctx.sessionController.resolveDurableSession({ sessionId, workspacePath })`. The Host validates the exact workspace, shares concurrent hydration, and publishes the persisted Session into the live registry without mounting an Agent, appending an event, marking list visibility, or calling a model. Persistence refuses logs that need crash repair rather than mutating them through this read-only path. `resolveDurableSessionSafe()` exposes the same operation as a bounded `{ ok, code }` result for Host integrations; it does not expose archive contents, locations, or underlying exception messages. Visibility remains an explicit later `markExternalTaskVisible` decision.
